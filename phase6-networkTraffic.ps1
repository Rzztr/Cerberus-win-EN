...
if ($IsWindows) {
    ...
    $connections | Select-Object LocalAddress, LocalPort, RemoteAddress, RemotePort,
    ...
    Get-NetTCPConnection | Where-Object State -eq "Established" | ForEach-Object {
        ...
        foreach ($range in $telegramRanges) {
            ...
            $mask = [int]$rangeParts[1]
            $match = $true
            for ($i=0;$i -lt 4;$i++) {
                $maskByte = 0xFF -shl (8-$mask)
                if ( ($ipBytes[$i] -band $maskByte) -ne ($rangeBytes[$i] -band $maskByte) ) {
                    $match = $false
                    break
                }
            }
    ...
    Write-Verbose "Skipping network traffic collection on non‑Windows OS"
}