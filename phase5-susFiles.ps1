...
Get-ChildItem "$env:APPDATA\*" -Include "*.exe", "*.dll", "*.scr", "*.pif" -Recurse -Force -ErrorAction SilentlyContinue |
    Select-Object FullName, Length, CreationTime, LastWriteTime | Export-Csv "$out\appdata_executables.csv" -NoTypeInformation

Get-ChildItem "$env:TEMP\*" -Include "*.exe", "*.dll", "*.ps1", "*.vbs", "*.js", "*.bat" -Recurse -Force -ErrorAction SilentlyContinue |
    Select-Object FullName, Length, CreationTime, LastWriteTime | Export-Csv "$out\temp_scripts.csv" -NoTypeInformation
...
Get-ChildItem "$env:USERPROFILE" -Force -Filter *.exe -Recurse -ErrorAction SilentlyContinue |
    ...
    Select-Object FullName, Length, CreationTime, LastWriteTime | Out-File "$out\hidden_executables.txt"
...
foreach ($proc in $browserProcs) {
    ...
    if ($p) {
        # Use WMI to get loaded modules (PowerShell alternative requires admin)
